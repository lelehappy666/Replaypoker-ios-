...
River Club 是一个面向 iPhone 横屏体验的德州扑克 UI 原型。项目以经典赌场氛围为视觉基础，使用深墨绿、胡桃木与古金色构建从登录、选桌、买入到九人牌局的完整体验路径。

当前版本专注于界面、导航、状态处理与可测试性，数据由本地 `MockPokerRepository` 提供。它不包含实时多人网络、扑克规则引擎、生产级身份认证或后端服务。
...
- **Repository 抽象**：`PokerRepository` 定义数据契约，`MockPokerRepository` 提供可重复的本地数据。
...
在 Xcode 中选择 `RiverClub` scheme 和 iPhone 16 Pro Max 模拟器后运行。应用只支持向左和向右横屏。
...
> 当前开发机的活动开发目录指向 Command Line Tools，而不是完整 Xcode，因此本次 README 更新未重新执行 iOS 测试；需在上述完整 Xcode 环境中运行。

## 当前范围

当前仓库是使用本地固定数据驱动的 UI 原型，已实现界面导航、列表筛选、买入校验、九人桌布局与异常状态展示。
...
1. 接入正式身份与玩家资料服务。
2. 为 Repository 增加网络实现和离线缓存策略。
3. 接入实时牌局状态同步与扑克规则引擎。
4. 增加设计资源、动态效果与更多无障碍验证。
5. 在完整 Xcode 环境中持续运行单元测试和 UI 测试。

## 免责声明

River Club 是用于界面设计与工程验证的娱乐型扑克原型。项目中的筹码没有现金价值，不能充值、提现、兑换或转化为任何真实货币或收益；项目不提供真钱扑克服务。
