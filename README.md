...
| 九人牌桌 | 九个座位、放大手牌与公共牌、实时中文牌型、赢家筹码动画、聊天入口、下注控制与安全离桌 |
| 本地牌局 | `PokerCore` 负责发牌、动作合法性、下注轮次、底池与结算，支持普通桌可玩闭环 |
| 机器人 | 机器人只依据自身安全观察和公开桌面信息决策；下注按底池比例计算，全下受牌力、有效筹码和赔率资格限制，不能读取其他玩家隐藏底牌 |
...
- 买入范围、余额不足、快速匹配和候补行为。
- 九个唯一座位与本地玩家底部居中规则。
- 大厅窄侧栏、放大牌面、圆形头像和实时牌型提示。
- 深筹码机器人不会无条件集体全下，短筹码与强牌仍可合理全下。
- 单赢家、多人平分和边池奖励事件正确移动筹码并高亮赢家。
- 进行中离桌会二次确认、自动弃牌、合法结算、幂等退款并返回来源页面。
- 多种安全画布尺寸下的横屏布局。
- 加载、空数据、离线缓存与失败重试状态。
- 核心游客流程和娱乐筹码合规性。
